...
describe("doublePointsByCount", () => {
  test("指定された回数だけポイントを倍増する", () => {
    const actual = doublePointsByCount(4);
    expect(actual).toBe(8);
  });

  test("指定された回数が 0 の場合は 0 を返す", () => {
    const actual = doublePointsByCount(0);
    expect(actual).toBe(0);
  });
});

test("スクラッチポイントの合計値を算出する", () => {
  const actual = calculateScratchcardPoints(sampleCards);
