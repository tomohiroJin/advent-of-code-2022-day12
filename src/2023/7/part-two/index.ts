...
export const determineHandType = (hand: string): number => {
  ...
  if (isJoker) {
    const jokers = cards.filter((pair) => pair[0] === "J")[0];
    cards = cards.filter((pair) => pair[0] !== "J");
    const maxPair = cards.reduce(
      (maxPair, pair) => (maxPair.length < pair.length ? pair : maxPair),
      []
    );
    cards = cards.map((pair) =>
      pair[0] === maxPair[0] ? [...pair, ...jokers] : pair
    );
    if (jokers.length === 5) {
      cards.push([...jokers]);
    }
...
export const calculateTotalEarningsFromHandRanks = (
  ...
  let minusIndex = 0;
  return sortByAscendingStrength(
    handBidList.split("\n").map((handBidRow) => extractHandAndBid(handBidRow))
  ).reduce((total, handBidPair, index, array) => {
    if (index > 0 && array[index - 1].hand === handBidPair.hand) {
      minusIndex -= 1;
    }
    total += handBidPair.bid * (index + 1 + minusIndex);
    return total;
  }, 0);
};
